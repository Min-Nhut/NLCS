...
class OutStandingDoctor extends Component {
    ...
    async componentDidMount() {
        ...
        try {
            ...
            if (res && res.data) {
                const specialtyNames = res.data.map((item) => item.name);
                this.setState({ specialtyName: specialtyNames });
            }
        } catch (error) {
    ...
    render() {
        ...
        const doctorsToShow =
            arrDoctors && arrDoctors.length > 0 ? arrDoctors.concat(arrDoctors).concat(arrDoctors) : [];
